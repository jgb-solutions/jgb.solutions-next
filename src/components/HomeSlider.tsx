...
export default () => (
  <Slider autoplay={2000} touchDisabled={false} >
    {ServiceList.map((item, index) => (
      <div className="bg-cover bg-center bg-repeat" key={index} style={{ background: `url('${item.image}')`, }}>
        <div className="bg-black bg-opacity-50 h-full flex flex-col justify-between items-end py-3 pr-3 text-white">
          ...
          <button className="bg-orange text-lg sm:p-6 p-3 rounded-large text-black sm:text-2xl font-bold uppercase"
          >{item.button}</button>
        </div>
      </div>
    ))}
